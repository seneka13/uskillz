export const sliderBtn = (navList, callback) => {
  let nav = navList.querySelectorAll("#btnTab");
  nav[0].click();
...
export const sliderBtnGraphs = (navList, callback) => {
  let nav = navList.querySelectorAll("#toggleGraphs");
  nav[0].click();
  let menuWidth = navList.offsetWidth;
  if (nav.length > 0) {
    let marginLeft = [];
